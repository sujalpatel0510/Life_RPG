...
export const EquippedLoadoutCard: React.FC<EquippedLoadoutCardProps> = ({ onOpenArmoury }) => {
  ...
  const getRarityGlow = (rarity: string) => {
    switch (rarity) {
      case 'COMMON': return 'border-slate-700 bg-slate-900/60';
      case 'UNCOMMON': return 'border-emerald-500/50 bg-emerald-950/20';
      case 'RARE': return 'border-blue-500/60 bg-blue-950/30 item-glow-rare';
      case 'EPIC': return 'border-purple-500/60 bg-purple-950/30 item-glow-epic';
      case 'LEGENDARY': return 'border-amber-500/70 bg-amber-950/30 item-glow-legendary animate-pulse-glow';
  ...
  return (
    <div className="bg-[#101626]/90 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
      <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
        <h3 className="font-fantasy font-bold text-sm text-slate-100 flex items-center gap-2">
          ...
          {slots.map((s) => {
            ...
            return (
              <button
                ...
                className="w-full rounded-2xl border border-dashed border-slate-800 hover:border-amber-500/40 bg-slate-900/30 p-3 flex items-center justify-between group transition text-left"
              >
                <div className="flex items-center space-x-3">
                  <div className="w-9 h-9 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-center text-slate-600 group-hover:text-amber-400/80 transition">
                    <Icon className="w-4 h-4" />
                  </div>
                  <div>
                ...
                <Plus className="w-4 h-4 text-slate-600 group-hover:text-amber-400 transition mr-1" />
              </button>
            );
          })}
        </div>
