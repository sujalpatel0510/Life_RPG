...
export const EquippedLoadoutCard: React.FC<EquippedLoadoutCardProps> = ({ onOpenArmoury }) => {
  ...
  const getRarityGlow = (rarity: string) => {
    switch (rarity) {
      ...
      case 'LEGENDARY': return 'border-amber-300 dark:border-amber-500/70 bg-amber-50/60 dark:bg-amber-950/30 item-glow-legendary animate-pulse-glow';
    ...
    <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800/80">
        ...
        <button
          ...
          className="text-[11px] font-semibold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 flex items-center gap-1"
        >
          <span>Armoury</span>
          ...
          {slots.map((s) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center space-x-3 min-w-0">
                    <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-950/80 border border-current flex items-center justify-center flex-shrink-0 text-amber-600 dark:text-amber-400 shadow-sm">
            ...
            return (
              <button
                ...
                className="w-full rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 hover:border-amber-400 dark:hover:border-amber-500/40 bg-slate-50 dark:bg-slate-900/30 p-3 flex items-center justify-between group transition text-left"
              >
                <div className="flex items-center space-x-3">
                  <div className="w-9 h-9 rounded-xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-600 group-hover:text-amber-500 transition shadow-sm">
                    <Icon className="w-4 h-4" />
                  </div>
                  <div>
                ...
                <Plus className="w-4 h-4 text-slate-400 dark:text-slate-600 group-hover:text-amber-500 transition mr-1" />
              </button>
            );
          })}
        </div>
