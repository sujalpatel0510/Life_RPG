...
export const QuestBoard: React.FC<QuestBoardProps> = ({ 
  ...
  return (
    <div className="space-y-6">
          ...
          <button
            ...
            className={`btn-tactile flex items-center space-x-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all ${
              activeCadence === c.id
                ...
                : 'surface text-body border border-slate-200 dark:border-[#252d4a] hover:border-indigo-400/60 hover:text-title dark:hover:text-[#f4f3f0] shadow-sm'
            }`}
          >
            <span>{c.label}</span>
            <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${
              activeCadence === c.id ? 'bg-black/30 text-white' : 'bg-slate-100 dark:bg-[#1d2340] text-slate-600 dark:text-slate-400'
            }`}>
              {c.count}
            </span>
          </button>
      ...
      {/* Category Pills */}
      <div className="flex items-center space-x-2 overflow-x-auto pb-1 no-scrollbar">
        <Filter className="w-3.5 h-3.5 text-muted flex-shrink-0 ml-1" />
        {categories.map((cat) => (
          <button
            key={cat.id}
            onClick={() => {
            ...
            className={`btn-tactile px-2.5 py-1 rounded-lg text-[11px] font-semibold whitespace-nowrap transition ${
              activeCategory === cat.id
                ...
                : 'text-slate-500 dark:text-slate-400 hover:text-title dark:hover:text-[#f4f3f0] bg-white/60 dark:bg-[#1d2340] border border-slate-200 dark:border-[#252d4a]'
            }`}
          >
            {cat.label}
          </button>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map((n) => (
            <div key={n} className="h-32 rounded-2xl border border-slate-200 dark:border-[#252d4a] skeleton" />
          ))}
        </div>
        ...
        /* Empty State */
        <div className="text-center py-16 px-4 rounded-3xl surface border-2 border-dashed border-slate-200 dark:border-[#252d4a] space-y-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-xs text-body max-w-sm mx-auto mt-1">
              Type in the Quick-Add bar above or press <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-[#1d2340] text-indigo-600 dark:text-indigo-400 font-mono text-[10px] border border-slate-200 dark:border-[#252d4a]">N</kbd> to summon your first objective.
            </p>
          </div>
        </div>
      )}
      ...
      {completedQuests.length > 0 && (
        <div className="pt-4 border-t border-slate-200 dark:border-[#252d4a]">
          <button
            type="button"
            onClick={() => {
              playClick();
            ...
            className="w-full flex items-center justify-between p-3.5 rounded-xl bg-slate-50 dark:bg-[#1d2340] hover:bg-slate-100 dark:hover:bg-[#1d2340]/80 border border-slate-200 dark:border-[#252d4a] text-body hover:text-title dark:hover:text-[#f4f3f0] transition"
          >
            <div className="flex items-center space-x-2 text-xs font-bold uppercase tracking-wider">
              <CheckCircle2 className="w-4 h-4 text-rpg-stats-vitality" />
